...
HighPrecision EqualTimeSolver::getEqualTime()
{
        ...
        auto h = [&](HighPrecision t) -> HighPrecision {
            ...
            auto val1_fut = std::async(std::launch::async, rho1, p, t);
            auto val2_fut = std::async(std::launch::async, rho2, p, t);
        ...
        HighPrecision rho2Equal = rho2(p, timeEquality);
        // Check for constraint
        if (hasRestriction == true && !(rho2Equal > restriction(p, timeEquality)))
        {
