...
EnergyDensity ChiParticle::energyDensityMatter(HighPrecision t0)
{
    return [this, t0](HighPrecision t)->HighPrecision{
        HighPrecision time = pow(t0, (2.0 / 3.0)) / pow(t, (2.0 / 3.0));
        HighPrecision initialRho = this->getInitialRhoMatter() * pow(time, 4);
        HighPrecision prefactor = pow(1 / pow(t, (2.0 / 3.0)), 4);
        EnergyDensity rhoMat = phiParticle->energyDensityMatter(t0);

        auto integrand = [&] (HighPrecision tprime)
        {
            HighPrecision val = ChiDecayRate(this->p, 4, t0, tprime) * rhoMat(tprime) * pow(tprime, (8.0 / 3.0));
            return val;
        };
...
EnergyDensity ChiParticle::energyDensityRadiation(HighPrecision t0)
{
    return [this, t0](HighPrecision t)->HighPrecision{
        ...
        auto integrand = [&] (HighPrecision tprime)
        {
            ...
            HighPrecision val = ChiDecayRate(this->p, 2, t0, tprime) * rhoRad(tprime) * pow(tprime, 2.0);
            return val;
        };
